...
void bake(int index = 0, int remaining = TEASPOONS, int c = 0, int d = 0, int f = 0, int t = 0) {
    if (index == INGREDIENTS - 1) {
        ...
        for (int i = 0; i <= remaining; i++) {
            bake(index + 1, remaining - i, c + i * capacity[index], d + i * durability[index], f + i * flavor[index], t + i * texture[index]);
        }
    }
}
